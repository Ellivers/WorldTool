# Called by worldtool:process/clone/common
...
execute if entity @s[tag=!wt.process.clone.filter.normal,tag=!wt.process.clone.filter.exclude] run function worldtool:process/place_block.primary
scoreboard players set #blockProcessed2 worldtool 1
