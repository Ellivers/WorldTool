...
kill @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer,sort=nearest,limit=1]
summon minecraft:area_effect_cloud ~ ~ ~ {Age: -2147483648, Duration: -1, WaitTime: -2147483648, Tags: ["wt_drawer","random1","from_cmd","worldtool"]}

execute as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer,tag=from_cmd] at @s run function worldtool:cmd/random/1