...
execute unless entity @s[tag=cloning_wt] run kill @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer,sort=nearest,limit=1]
execute unless entity @s[tag=cloning_wt] run summon minecraft:area_effect_cloud ~ ~ ~ {Age: -2147483648, Duration: -1, WaitTime: -2147483648, Tags: ["wt_drawer","from_cmd","ticking","worldtool"]}
...
execute if score @s wt_chance = @s wt_chance run scoreboard players operation @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer,sort=nearest,limit=1] wt_chance = @s wt_chance
tag @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer,sort=nearest,limit=1] remove cmdpos1

execute if entity @s[tag=cloning_wt] run function worldtool:cmd/load_clone1
execute if entity @s[tag=random1] as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=cmdrnd1] at @s run function worldtool:cmd/start_random_1
