...
execute if entity @s[tag=atsetpos] run function worldtool:ui_general/clone/remove_preview
...
execute if entity @s[tag=!atsetpos] as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1] if score @s wt_ID = #ID_temp worldtool run function worldtool:ui_general/clone/select_clone.pos1_1
execute if entity @s[tag=atsetpos] as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1] if score @s wt_ID = #ID_temp worldtool run function worldtool:ui_general/clone/select_clone.pos1_2

execute if entity @s[tag=!atsetpos,tag=wt_outline_selection] if score $particles worldtool matches ..0 run tellraw @s [{"text": "\nCouldn't display outlining particles, as particles are disabled. ","color":"red"},{"text": "[Enable]","color":"white","hoverEvent": {"action": "show_text","value": "Enable WorldTool particles"},"clickEvent": {"action":"run_command","value": "/function worldtool:ui_general/options/particles_toggle"}}]
