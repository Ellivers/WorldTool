...
execute unless data storage worldtool:storage {Language:"worldtool:en_us"} run tellraw @s {"text":"[English (US)]","color": "green","hoverEvent": {"action": "show_text","contents": {"nbt":"Translation.\"button.settings.select_language.description\"","storage": "worldtool:storage"}},"clickEvent": {"action": "run_command","value": "/function worldtool:ui_general/options/settings/language/set/en_us"}}
