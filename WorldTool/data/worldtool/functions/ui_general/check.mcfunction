...
execute if entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1_found] if entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos2_found] run function worldtool:ui_general/select

# If the menu isn't open (AKA both positions aren't there)
execute unless entity @s[tag=in_menu] run function worldtool:ui_general/error.pos_missing
...
execute if score #pos1Count worldtool matches 2.. run function worldtool:ui_general/error.too_many_pos1
...
execute if score #pos2Count worldtool matches 2.. run function worldtool:ui_general/error.too_many_pos2
