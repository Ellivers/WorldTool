...
execute if entity @s[tag=wt.block.primary] if data storage worldtool:storage Temp.Block.State{Name:"minecraft:air"} run setblock 0 1 13 minecraft:air
...
execute if entity @s[tag=wt.block.secondary] if data storage worldtool:storage Temp.Block.State{Name:"minecraft:air"} run setblock 2 1 13 minecraft:air

summon minecraft:falling_block 2 1 13 {Time:-34,Tags:["worldtool","wt.update_block"]}

execute positioned 2 1 13 run data modify entity @e[type=minecraft:falling_block,tag=worldtool,tag=wt.update_block,sort=nearest,limit=1] BlockState set from storage worldtool:storage Temp.Block.State
execute if data storage worldtool:storage Temp.Block.Data positioned 2 1 13 run data modify entity @e[type=minecraft:falling_block,tag=worldtool,tag=wt.update_block,sort=nearest,limit=1] TileEntityData set from storage worldtool:storage Temp.Block.Data
