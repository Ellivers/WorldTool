...
execute as @a if score @s wt_ID = #ID_temp worldtool at @s run function worldtool:process_load/clone/set_temp_score
...
execute if entity @s[tag=!mirrorY] unless score @s wt_rotX matches -90.. unless score @s wt_rotZ matches -90.. if score $pos1y worldtool > $pos2y worldtool run function worldtool:process_load/switch_y_levels2
