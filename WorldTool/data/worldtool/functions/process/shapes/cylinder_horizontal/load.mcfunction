...
execute if score #shapeGenStatus_temp worldtool matches 1 unless score #blocksChecked worldtool >= #blocksPerTick worldtool unless score #lengthReached worldtool >= #length_temp worldtool at @s positioned ^1 ^ ^ run function worldtool:process/shapes/cylinder_horizontal/loop
