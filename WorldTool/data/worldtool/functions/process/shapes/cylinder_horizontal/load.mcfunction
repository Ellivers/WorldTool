...
execute if score #shapeGenStatus_temp worldtool matches 0 unless score #blocksChecked worldtool >= #blocksPerTick worldtool unless score #lengthReached worldtool >= #fullLength worldtool at @s positioned ^1 ^ ^ run function worldtool:process/shapes/cylinder_horizontal/loop
execute if score #shapeGenStatus_temp worldtool matches 1 run function worldtool:process/shapes/cylinder_horizontal/loop
