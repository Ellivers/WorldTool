...
execute unless data storage worldtool:storage Processes[-1].Owner run data modify storage worldtool:storage Temp.LogEntry set value {Message:'{"nbt":"Translation.\\"info.process_entity_found\\"","storage":"worldtool:storage"}',Type:"info"}
execute unless data storage worldtool:storage Processes[-1].Owner run function worldtool:technical/cmd/log/add
