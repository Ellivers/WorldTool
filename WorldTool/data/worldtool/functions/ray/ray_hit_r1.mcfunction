execute at @s positioned ~ ~ ~ as @e[type=minecraft:area_effect_cloud,tag=randomcorner1] if score @s wt_ID = @p wt_ID at @s run function worldtool:ray/del_pos
...
execute if score $forceLoadPositions worldtool matches 1.. store success score @e[type=minecraft:area_effect_cloud,tag=randomcorner1,sort=nearest,limit=1] worldtool run forceload add ~ ~
