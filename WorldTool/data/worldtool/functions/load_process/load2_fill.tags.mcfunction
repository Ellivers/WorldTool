...
execute if entity @s[tag=!cmd_block_fill,tag=!cloning_wt,tag=!cmdlist_search,tag=!cmdlist,tag=!hollow,tag=!random1,tag=!random2,tag=!measuring,tag=!brushing,tag=!wt_destroy,tag=!foliage_ing,tag=!checkered,tag=!filling_outline] run function worldtool:fill_v2/x
...
execute unless entity @s[tag=!cloning_wt,tag=!hollow,tag=!random1,tag=!random2,tag=!foliage_ing,tag=!filling_outline,tag=!wt_circlegen] run function worldtool:load_process/load2_fill.tags1
