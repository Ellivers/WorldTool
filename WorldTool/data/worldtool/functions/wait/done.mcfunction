...
execute if entity @s[tag=!wt_random_2] as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1,tag=!cloning_wt] if score @s wt_ID = #ID_temp worldtool as @p run function worldtool:load_process/global_start
...
execute if entity @s[tag=wt_random_2] as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=randomcorner1] if score @s wt_ID = #ID_temp worldtool at @s run function worldtool:load_process/randomize/load
