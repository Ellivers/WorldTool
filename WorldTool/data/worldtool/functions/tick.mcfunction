...
execute if score $functionRunning worldtool matches 1 unless score #drawerError worldtool matches 1 unless entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer] as @a[tag=wt_started_function,limit=1] run function worldtool:ui/error.no_drawer
execute if score $functionRunning worldtool matches 1 if score #drawerError worldtool matches 1 if entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_drawer] as @a[tag=wt_started_function,limit=1] run function worldtool:ui/drawer_found
