...
execute if block ~ ~ ~ #worldtool:cmd_block_chain_start run function worldtool:cmd_list/read/start

#If #blockschecked's worldtool score isn't more than a certain number (default: 4000) move the x, y and z coordinates
execute unless block ~ ~ ~ #worldtool:cmd_block_chain_start if score $drawingposX worldtool < $pos2x worldtool unless score #blockschecked worldtool >= $blocksPerTick worldtool run function worldtool:cmd_list/search/x
execute unless block ~ ~ ~ #worldtool:cmd_block_chain_start if score $drawingposX worldtool > $pos2x worldtool unless score #blockschecked worldtool >= $blocksPerTick worldtool run function worldtool:cmd_list/search/-x

execute unless block ~ ~ ~ #worldtool:cmd_block_chain_start if score $drawingposX worldtool = $pos2x worldtool if score $drawingposY worldtool < $pos2y worldtool unless score #blockschecked worldtool >= $blocksPerTick worldtool run function worldtool:cmd_list/search/y
execute unless block ~ ~ ~ #worldtool:cmd_block_chain_start if score $drawingposX worldtool = $pos2x worldtool if score $drawingposY worldtool > $pos2y worldtool unless score #blockschecked worldtool >= $blocksPerTick worldtool run function worldtool:cmd_list/search/-y
