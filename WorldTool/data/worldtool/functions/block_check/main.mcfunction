...
execute unless block ~1 ~ ~ minecraft:air at @a[tag=placing_fill] if score @s wt_ID = @p wt_ID as @p run function worldtool:load_function/start_check
...
execute unless block ~1 ~ ~ minecraft:air at @a[tag=replacing_fill2] if score @s wt_ID = @p wt_ID as @p run function worldtool:load_function/start_check
