...
execute as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker] if score @s wt_ID = #ID_temp worldtool run tag @s add check_clone_overlap

# Get position of position 1
execute store result score @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap,limit=1] wt_pos1x as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1] if score @s wt_ID = #ID_temp worldtool run data get entity @s Pos[0]
execute store result score @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap,limit=1] wt_pos1y as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1] if score @s wt_ID = #ID_temp worldtool run data get entity @s Pos[1]
execute store result score @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap,limit=1] wt_pos1z as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos1] if score @s wt_ID = #ID_temp worldtool run data get entity @s Pos[2]

# Get position of position 2
execute store result score @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap,limit=1] wt_pos2x as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos2] if score @s wt_ID = #ID_temp worldtool run data get entity @s Pos[0]
execute store result score @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap,limit=1] wt_pos2y as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos2] if score @s wt_ID = #ID_temp worldtool run data get entity @s Pos[1]
execute store result score @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap,limit=1] wt_pos2z as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=pos2] if score @s wt_ID = #ID_temp worldtool run data get entity @s Pos[2]
...
execute at @s as @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_waiting_marker,tag=check_clone_overlap] if score @s wt_ID = #ID_temp worldtool run function worldtool:processes/clone/check_overlap/set_cpos2
