...
execute if score $forceLoadPositions worldtool matches 0 as @e[type=minecraft:area_effect_cloud] unless score @s worldtool matches 0 unless entity @s[tag=!pos1,tag=!pos2] at @s run forceload remove ~ ~ 
execute if score $forceLoadPositions worldtool matches 1 as @e[type=minecraft:area_effect_cloud] unless entity @s[tag=!pos1,tag=!pos2] store success score @s worldtool run forceload add ~ ~
