...
execute if entity @s[tag=wt_highBPT] run tellraw @s ["",{"text": "Your world seems so be lagging. To reduce the amount of blocks read per tick to Medium (currently set to High), ","color":"gold"},{"text": "[click here]","color":"green","hoverEvent": {"action": "show_text","value": "Set to Medium"},"clickEvent": {"action": "run_command","value": "/function worldtool:ui/lag_warning/set_medium"}}]
execute if entity @s[tag=wt_mediumBPT] run tellraw @s ["",{"text": "Your world seems so be lagging. To reduce the amount of blocks read per tick to Low (currently set to Medium), ","color":"gold"},{"text": "[click here]","color":"green","hoverEvent": {"action": "show_text","value": "Set to Low"},"clickEvent": {"action": "run_command","value": "/function worldtool:ui/lag_warning/set_low"}}]
