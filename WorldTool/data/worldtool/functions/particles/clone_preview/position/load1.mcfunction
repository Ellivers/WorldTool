execute at @s as @e[type=minecraft:area_effect_cloud,tag=worldtool] unless entity @s[tag=!wt_clone_preview_pos1,tag=!wt_clone_preview_pos2] if score @s wt_ID = @p wt_ID run tag @s add outline_load
