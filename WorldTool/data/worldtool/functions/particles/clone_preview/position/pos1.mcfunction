...
execute store result score @s wt_pos2x run data get entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_clone_preview_pos2,tag=outline_load,limit=1] Pos[0]
execute store result score @s wt_pos2y run data get entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_clone_preview_pos2,tag=outline_load,limit=1] Pos[1]
execute store result score @s wt_pos2z run data get entity @e[type=minecraft:area_effect_cloud,tag=worldtool,tag=wt_clone_preview_pos2,tag=outline_load,limit=1] Pos[2]
...
execute at @s if score @s wt_pos1x = @s wt_pos2x run tp @s ~.999 ~ ~
execute at @s if score @s wt_pos1y = @s wt_pos2y run tp @s ~ ~.999 ~
execute at @s if score @s wt_pos1z = @s wt_pos2z run tp @s ~ ~ ~.999
