...
execute unless entity @s[tag=!pos1,tag=!pos2] at @a[tag=in_menu] if score @p wt_ID = #ID_temp worldtool at @s run function worldtool:particles/mark_selection/load
execute unless entity @s[tag=!wt_clone_preview_pos1,tag=!wt_clone_preview_pos2] at @a[tag=in_menu] if score @p wt_ID = #ID_temp worldtool at @s run function worldtool:particles/clone_preview/load
