...
execute as @a[tag=in_menu] if score @s wt_ID = #ID_temp worldtool run tag @s add wt_particle_user
...
execute unless entity @s[tag=!wt_clone_preview_pos1,tag=!wt_clone_preview_pos2] at @a[tag=in_menu] if entity @a[tag=wt_particle_user] run function worldtool:particles/clone_preview/load
