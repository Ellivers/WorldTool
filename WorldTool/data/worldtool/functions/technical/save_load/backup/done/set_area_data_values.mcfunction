...
# Sets the position and size of saved areas

data modify storage rx.playerdb:io player.data.WorldTool.Current append value {Pos:[0d,0d,0d],Size:[0,0,0],TemplateList:[]}

data modify storage rx.playerdb:io player.data.WorldTool.Current[-1].TemplateList set from storage worldtool:storage Processes[-1].Output.SavedAreas[0].TemplateList
data modify storage rx.playerdb:io player.data.WorldTool.Current[-1].Pos set from storage worldtool:storage Processes[-1].Output.SavedAreas[0].Positions.1
...
execute store result storage rx.playerdb:io player.data.WorldTool.Current[-1].Size[0] int 1 run scoreboard players get #sizeX worldtool
execute store result storage rx.playerdb:io player.data.WorldTool.Current[-1].Size[1] int 1 run scoreboard players get #sizeY worldtool
execute store result storage rx.playerdb:io player.data.WorldTool.Current[-1].Size[2] int 1 run scoreboard players get #sizeZ worldtool
...
execute if data storage worldtool:storage Processes[-1].Output.SavedAreas[0] run function worldtool:technical/save_load/backup/done/set_area_data_values
