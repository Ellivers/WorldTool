...
execute if score #success worldtool matches 1 if score $enableBackups worldtool matches 1 if score $playerdbAvailable worldtool matches 1 unless data storage worldtool:storage Temp.Process{Tags:["wt.read_only"]} run function worldtool:ui_general/options/save_load/backup/setup_process
